 ...
 *   - Anything stated as a LINE RANGE. The README used to say `sed -n '21,60p'
 *     .../dockerBuilder.ts` and "one 40-line function"; both were exact at the
 *     commit they were written on and both were stale within days, because a
 *     line range goes wrong on any edit above or inside the function. Pinning
 *     them here would only encode a transient working-tree state. They were
 *     reworded to a symbol-anchored `sed -n '/^export function .../,/^}$/p'`
 *     instead, which is a prose fix rather than a checker feature — and the
 *     reason every anchor in this file matches a declaration or a sentence and
 *     never a position in a file.
 *   - Paths and file names. "`documentSearchService.ts` is one cosine query",
 *     "the validator lives at `services/sqlValidator.ts`" — real claims, and
 *     two of them were wrong when this file was ported onto the current
 *     lineage. They are prose, not numbers, and a `--check` that is green says
 *     nothing about them. Read them.
 *
 * USAGE
...
/** The hardening flags the README's Sandboxing section claims are absent. */
const ABSENT_SANDBOX_FLAGS = ['pids-limit', 'cap-drop', 'no-new-privileges', 'seccomp'];

const CI_WORKFLOW = '.github/workflows/ci.yml';
/** The commands the README's Testing section tells the reader CI runs. */
const CI_STEPS_README_CLAIMS = [
  'npm run check:system-card',
  'npm run test:landing',
  'npm run readme:check',
];

/**
 * `git grep -c <flag> -- backend deploy`, as a tri-state rather than a boolean.
 *
 * git grep exits 1 for "found nothing" and 0 for "found something", which is
 * exactly the distinction the absence invariant below is built on. It also
 * exits 128 for "not a git repository", for a worktree whose gitdir pointer
 * does not resolve, and for a bad pathspec.
 *
 * This used to be `execSync('git grep -c ... || true')`, which flattened all
 * three into an empty string — so any failure of the command became the answer
 * that makes the invariant pass, and the check reported "absent" without ever
 * having looked. That is the failure this whole file exists to prevent, sitting
 * inside the file.
 *
 * Be precise about the exposure, because overstating it would be the same sin.
 * Measured: a missing pathspec exits 1, not 128 — `git grep -c x -- nosuchdir`
 * is indistinguishable from a genuine no-match. The 128s come from "not a git
 * repository", a gitdir pointer that does not resolve, dubious ownership. In
 * today's file those would already have thrown out of `lsFiles`, which
 * `resolveFacts()` runs before any invariant, so the old form was latent rather
 * than live: no green run was actually bought by it. It was still the only git
 * call here that swallowed its own errors, and the flag list below is data
 * somebody will extend. A check that turns its breakage into a pass is worse
 * than an absent one, because it looks like evidence. Now exit 1 is an answer
 * and every other non-zero exit is an error.
 */
function gitGrepCount(flag) {
  try {
    return execSync(`git grep -c "${flag}" -- backend deploy`, {
      cwd: REPO,
      encoding: 'utf8',
    ...
    throw new Error(
      `git grep for "${flag}" could not run (exit ${e.status}): ` +
        `${String(e.stderr || e.message).trim()}. The sandbox-absence invariant ` +
        `cannot be evaluated, so this run proves nothing and fails rather than passing.`
    );
  }
}
...
const FACTS = {
  ...
  mcpTools: {
    ...
    sites: [
      ...
      /* Anchored on the diagram's own header sentence, which names the count
         and the file it came from. Neither booklet file may be EDITED to suit
         this checker: scripts/check-system-card.mjs hashes booklet/src against
         landing/public/system-card/, so a one-word change here turns that gate
         red. The regex moves; the sources do not. */
      { re: /hub with the (\d+) tools the real/g, file: 'booklet/src/diagrams/MCPToolRegistry.tsx' },
    ],
  },
  toolArrays: {
    kind: 'static',
...
const INVARIANTS = [
  ...
  },
  {
    ...
    holds: () => ABSENT_SANDBOX_FLAGS.every((flag) => gitGrepCount(flag) === ''),
    explain: () => {
      const found = ABSENT_SANDBOX_FLAGS.filter((flag) => gitGrepCount(flag));
      return (
  ...
  {
    name: 'CI still runs the three steps the Testing section says it runs',
    /* The Testing section describes CI's shape, and that description is the
       part of this README with the worst track record: the version this file
       arrived with said "landing tests never run in CI", which was true of the
       tree it was written against and false of this one — landing installs,
       builds and tests here, and the system-card gate runs before everything.
       Prose about a workflow file drifts the moment the workflow changes, and
       nothing noticed for months. So the description gets an anchor too.

       Deliberately a substring check on the command strings the README prints,
       not a YAML parse: the claim is "CI runs this command", and the command
       text is exactly what would go missing. */
    holds: () => CI_STEPS_README_CLAIMS.every((step) => read(CI_WORKFLOW).includes(step)),
    explain: () => {
      const missing = CI_STEPS_README_CLAIMS.filter((step) => !read(CI_WORKFLOW).includes(step));
      return (
        `${CI_WORKFLOW} no longer runs: ${missing.join(', ')}. The README's Testing section ` +
        `still tells the reader it does. Either restore the step or rewrite the paragraph.`
      );
    },
  },
];
