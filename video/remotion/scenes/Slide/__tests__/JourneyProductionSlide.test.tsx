import { describe, expect, it, vi } from "vitest";

vi.mock("../../../../config/fonts", () => ({
  REGULAR_FONT: { fontFamily: "sans-serif", fontWeight: 500 },
  TITLE_FONT: { fontFamily: "sans-serif", fontWeight: 700 },
  SERIF_FONT: { fontFamily: "serif", fontWeight: 400 },
  MONOSPACE_FONT: { fontFamily: "monospace", fontWeight: 500 },
  ENDCARD_FONT: { fontFamily: "sans-serif", fontWeight: 600 },
  waitForFonts: async () => {},
}));

vi.mock("remotion", async () => {
  const actual = await vi.importActual<typeof import("remotion")>("remotion");
  return {
    ...actual,
    useCurrentFrame: vi.fn(() => 0),
    useVideoConfig: vi.fn(() => ({ fps: 60, width: 1920, height: 1080, durationInFrames: 480 })),
  };
});

vi.mock("../../../primitives/SlideShell", () => ({
  SlideShell: ({ eyebrow, children }: { eyebrow?: string; children: React.ReactNode }) => (
    <div data-testid="slide-shell">
      {eyebrow ? <div data-testid="eyebrow">{eyebrow}</div> : null}
      {children}
    </div>
  ),
}));

import React from "react";
import { renderToStaticMarkup } from "react-dom/server";
import { useCurrentFrame } from "remotion";
import { JourneyProductionSlide } from "../JourneyProductionSlide";

const renderAt = (frame: number) => {
  vi.mocked(useCurrentFrame).mockReturnValue(frame);
  return renderToStaticMarkup(<JourneyProductionSlide theme="light" meta={undefined} />);
};

describe("<JourneyProductionSlide />", () => {
  it("renders without throwing across phase-boundary frames", () => {
    for (const frame of [0, 30, 90, 130, 220, 310, 390, 479]) {
      expect(() => renderAt(frame)).not.toThrow();
    }
  });

  it("renders the SPRINTS 9-11 · PRODUCTION active header cell at frame 30+", () => {
    const markup = renderAt(60);
    expect(markup).toContain("SPRINTS 9-11");
    expect(markup).toContain("PRODUCTION");
  });

  it("renders the 'Three sprints to ship' serif hero line", () => {
    const markup = renderAt(140);
    expect(markup).toContain("Three sprints to ship");
  });

  it("renders all three production milestone labels at frame 390+", () => {
    const markup = renderAt(410);
    expect(markup).toContain("experiments phase");
    expect(markup).toContain("benchmarks + Remotion reel");
    expect(markup).toContain("151 commits in a single day");
  });

  it("renders the ScaleInNumber '151' hero numeral at hero-moment frame", () => {
    const markup = renderAt(450);
    expect(markup).toContain("151");
    expect(markup).toContain("transform:scale(");
  });

  it("renders the '2026-04-14' caption on the hero moment", () => {
    const markup = renderAt(450);
    expect(markup).toContain("2026-04-14");
  });

  it("references the amberBright accent in markup", () => {
    const markup = renderAt(450).toLowerCase();
    expect(markup).toContain("#f59e0b");
  });
});
